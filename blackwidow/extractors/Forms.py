...
def parse_form(el, driver):
    ...
    for iel in inputs:
        ...
        try:
            ...
            if iel.get_attribute("name"):
                ...
                if iel.get_attribute("id"):
                    tmp['name'] = iel.get_attribute("id")
            if iel.get_attribute("value"):
                tmp['value'] = iel.get_attribute("value")
            if iel.get_attribute("checked"):
                tmp['checked'] = True
            if iel.get_attribute("pattern"):
                tmp['pattern'] = iel.get_attribute("pattern")
        ...
        # pattern in checkbox might cause a problem...
        if "pattern" in tmp:
            form.add_pattern_input(tmp['type'], tmp['name'], tmp['value'], tmp['pattern'])
        else:
    ...
    for select in selects:
        ...
        if select.get_attribute("name"):
            ...
            tmp['name'] = select.get_attribute("id")
