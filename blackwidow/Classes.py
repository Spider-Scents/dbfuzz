...
# https://github.com/mirdeger/bw/blob/main/Classes.py
root_dirpath = os.path.dirname(os.path.abspath(__file__))
import logging
logpath = os.path.join(root_dirpath, 'logs')
Path(logpath).mkdir(parents=True, exist_ok=True)
log_file = os.path.join(os.path.join(logpath), 'crawl-' + str(time.time()) + '.log')
...
# TODO Do we need force=True ? # logging.basicConfig(filename=log_file, format='%(asctime)s\t%(name)s\t%(levelname)s\t[%(filename)s:%(lineno)d]\t%(message)s', datefmt='%Y-%m-%d:%H:%M:%S', level=logging.DEBUG, force=True)
...
class Form:
    ...
    class Element:
        def __init__(self, itype, name, value, pattern=None):
            self.itype =  itype
            self.name  =  name
            self.value =  value
            self.pattern = pattern
        def __repr__(self):
            return str( (self.itype, self.name, self.value, self.pattern) )
        def __eq__(self, other):
    ...
    def add_pattern_input(self, itype, name, value, pattern):
        new_el = self.Element(itype, name, value, pattern)
        key    = self.Element(itype, name, value, None)
...
class Crawler:
    def __init__(self, driver, url, urls=None, cookies=None, tokens=None):
        self.driver = driver
        # Start url
        self.url = url

        # DBFUZZ
        self.dbfuzz_tokens = tokens
        self.shallow = False

        self.use_queue = False
        self.url_queue = []
        if urls:
            self.url_queue = [url for url in urls if url and url.strip()]
            self.use_queue = True

        if cookies:
            driver.get(url)
            for cookie in cookies.split("; "):
                (k,v) = cookie.split("=", 1) # values can include '='
                print("adding cookie ",k, "=", v)
                driver.add_cookie({"name": k, "value": v})
                # TODO handle path driver.add_cookie({"name": k, "value": v, "path": "/osc/"})
        ####
    ...
    def start(self, debug_mode=False):
        ...
        while still_work:
            ...
            try:
                ...
                try:
                    ...
                    logging.info("(DBFUZZ)REC CRAWL DONE")
        ...
        logging.info("(DBFUZZ)DONE CRAWLING")
        print("Done crawling, ready to attack!")

        print("Skipping post-crawl attack for SMT-page-generator-experiments")
        self.driver.close()
        self.driver.quit()
        exit()
        # self.attack()
    ...
    def fix_form(self, form, payload_template, safe_attack):
        ...
        for parameter in form.inputs:
            ...
            if safe_attack:
                ...
                # if parameter.itype in ["text", "textarea", "password", "email", "url", "search"]:
                if True:
                    # Arm the payload
    ...
    def get_payloads(self):
        ...
        #                 ]
        # xss_payloads = ["<%LETTERS>",
        #                 "<script>xss("+alert_text+")</script>",
        #                 "<script>xss('%LETTERS')</script>"]

        xss_payloads = ["<script>xss("+alert_text+")</script>",
                        "<script>xss(\"%LETTERS\")</script>",
                        "\" onerror=\"xss("+alert_text+")\""]
    ...
    def arm_payload(self, payload_template):
        ...
        if "%RAND" in payload_template:
            lookup_id = str(random.randint(1,100000000))
            payload = payload_template.replace("%RAND", lookup_id)
        elif "%LETTERS" in payload_template:
            lookup_id = "".join( random.choice(string.ascii_letters) for x in range(5) )
            payload = payload_template.replace("%LETTERS", lookup_id)
    ...
    def inspect_attack(self, vector_edge):
        ...
        attribute_injects = self.driver.find_elements("xpath", "//*[@jaekpot-attribute]")
        ...
        # Tag injections
        lookup_ids = self.attack_lookup_table.keys()
        for lid in lookup_ids:
            try:
                injected_tags = self.driver.find_elements("tag name", lid)
                if injected_tags:
                    successful_xss.add(lid)
                    self.reflected_payload(lid, vector_edge)
            except:
                # print("No XSS tag found")
                pass
        ...
        if lookup_ids:
          ...
          print(lookup_ids)
          # We want to print the JSON data to stdout
          return_data = {"url": self.driver.current_url, "xss_ids": lookup_ids}
          print("DBFUZZHEADER" + json.dumps(return_data))
          print("== /DBFUZZ ==")
    ...
    def inspect_tracker(self, vector_edge):
        try:
            ...
            # DBFUZZ
            # Look for things matching our tokens
            # current token pattern is just 6 digits
            matches = re.findall("[^\d]9(\d{5})[^\d]", body_text)
            # WARNING buggy, does not capture the 9 in the token
            # fix on visualization side to not invalidate past results
            if matches:
                unique_matches = list(set(matches))
                print("== DBFUZZ ==")
                # We want to print the JSON data to stdout
                return_data = {"url": self.driver.current_url, "token_ids": unique_matches}
                print("DBFUZZHEADER" + json.dumps(return_data))
                print("== /DBFUZZ ==")

            # Search for known trackers
            if self.dbfuzz_tokens:
              for tracker in self.dbfuzz_tokens:
                  if tracker in body_text:
                      print("== DBFUZZ ==")
                      print("Found tracker: ", tracker)
                      return_data = {"url": self.driver.current_url, "input_token": tracker}
                      print("DBFUZZHEADER" + json.dumps(return_data))
                      print("== /DBFUZZ ==")



            ### No need for normal BW tracking in the DBFUZZ version
            #
    ...
    def next_unvisited_edge(self, driver, graph):
        ...
        # DBFUZZ. Handle list of URLs from args
        if os.path.exists("please_die_bw.txt") and open("please_die_bw.txt").read() == "1":
            open("please_die_bw.txt", "w+").write("0")
            driver.close()
            driver.quit()
            sys.exit()
        ...
        if self.use_queue:
            if self.url_queue:
                print("(DBFUZZ) In self.url_queue")
                next_url_from_queue = self.url_queue.pop(0)
                print("(DBFUZZ) Next URL from queue: ", next_url_from_queue)
                req = Request(next_url_from_queue,"get")
                current_cookies = driver.get_cookies()

                print("(DBFUZZ) Making a new edge")
                new_edge = graph.create_edge(self.root_req, req, CrawlEdge(req.method, None, current_cookies), None )
                graph.add(req)
                graph.connect(self.root_req, req, CrawlEdge(req.method, None, current_cookies), None )


                print("SCANNING ", new_edge, "FROM QUEUE")
                successful = follow_edge(driver, graph, new_edge)
                print("(DBFUZZ) Edge followed...")
                if successful:
                    print("(DBFUZZ) Edge followed... SUCCESS!")
                    return new_edge
                print("This is bad...")

            else:
                print("QUEUE IS EMPTY QUITTING")
                print("DBFUZZHEADER" + json.dumps({"done": 1}))
                driver.close()
                driver.quit()
                sys.exit()
        ...
        if self.shallow or not self.debug_mode:
            ...
            if self.early_gets == self.max_early_gets:
                ...
                # If not shallow we reset and continue scanning
        ...
        if not list_to_use:
            ...
            if not list_to_use:
                if random_int >= 0 and random_int < 1:
                    print("Looking for form")
                    list_to_use = [edge for edge in graph.edges if edge.value.method == "form" and edge.visited == False]
                elif random_int >= 1 and random_int < 90:
    ...
    def rec_crawl(self):
        ...
        if not todo:
            ...
            for node in graph.nodes:
                if node.value.url != "ROOTREQ" and node.value.url is not None:
                    export_urls.add(node.value.url)
                    # somehow a None is getting in here when initial scanning Wordpress
                    # TODO why?
                    # print(node.value.url)

            print("URLS-------")
            sorted_urls = linkrank(export_urls, [], True)
            for u in sorted_urls:
                # Not to confuse with 'url' in reflection
                return_data = {"seen_url": u}
                print("DBFUZZHEADER" + json.dumps(return_data))
                # print(u)
            print("DBFUZZHEADER" + json.dumps({"done": 1}))
            self.driver.close()
        ...
        # Add values to forms (including smt solving)
